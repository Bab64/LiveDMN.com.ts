...
import * as node_wav from 'node-wav';
...
export class AudioModel {
     ...
     * as input tensors
     */
    constructor(inputShape: number[], private labels: string[], private dataset: Dataset, private featureExtractor: WavFileFeatureExtractor) {
    ...
    private loadDataArray(dir: string, callback: Function) {
        return new Promise<Float32Array[][]>((resolve, reject) => {
            file_system.readdir(dir, (err, filenames) => {
                if (err)
                    reject(err);
    ...
    private decode(filename: string) {
        const result = node_wav.decode(file_system.readFileSync(filename));
